...
class AdminCulturalClubPage extends StatefulWidget {
  const AdminCulturalClubPage({Key? key}) : super(key: key);

  @override
  _AdminCulturalClubPageState createState() => _AdminCulturalClubPageState();
}

class _AdminCulturalClubPageState extends State<AdminCulturalClubPage> {
  ...
  Future<void> _pickImage({required bool isHeaderImage}) async {
  ...
  Future<void> _updateHeader() async {
    ...
    if (_headerImageFile != null) {
      imageUrl =
          await _uploadImage(_headerImageFile!, 'cultural_club/header_images');
    }

    try {
      await _firestore.collection('cultural_club').doc('header').set({
  ...
  Future<void> _updateFooter() async {
    try {
      await _firestore.collection('cultural_club').doc('footer').set({
  ...
  Future<void> _addEvent(BuildContext context) async {
    ...
    if (_eventImageFile != null) {
      imageUrl =
          await _uploadImage(_eventImageFile!, 'cultural_club/event_images');
    }
        ...
        builder: (context, setState) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              ElevatedButton(
                onPressed: () async {
                  await _pickImage(isHeaderImage: false);
        ...
        builder: (context, setState) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              ElevatedButton(
                onPressed: () async {
                  await _pickImage(isHeaderImage: true);
                  setState(() {}); // Refresh dialog to show selected image
                },
                child: const Text('Pick Image'),
              ),
  ...
  void _showUpdateFooterDialog() {
    // Load current footer data
    _firestore.collection('cultural_club').doc('footer').get().then((doc) {
      if (doc.exists) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Admin Cultural Club Page'),
      ),
